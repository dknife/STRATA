"""TCM2: D-STORM-Dense — NumPy BLAS matmul + Heaviside pruning (Mac).

Dense frontier propagation using Accelerate BLAS on macOS.
Same D-STORM algebra as TCM1 but operating on dense numpy arrays.
"""
...
class DenseStormIterator:
    """Dense BLAS-based reachability iterator (D-STORM-Dense).
...
def run_apsp(A_csr, k=-1, verbose=True):
    """APSP via D-STORM dense (Accelerate BLAS matmul on Mac)."""
    ...
    n = len(A)
    if verbose:
        print(f"  TCM2 D-STORM-Dense: n={n}")
