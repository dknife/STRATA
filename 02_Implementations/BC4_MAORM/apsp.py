"""BC4: M-AORM — Matrix-multiply AORM (dense BLAS matmul).

Original AORM framework (IEEE Access, 2021) using dense
matrix multiplication (numpy BLAS) for R^k computation.
"""
...
class MAORMIterator:
    ...
    (SIMD/BLAS accelerated), then applies Heaviside pruning.
    """
...
def run_apsp(A_csr, k=-1, verbose=True):
    """APSP via M-AORM (dense BLAS matmul)."""
