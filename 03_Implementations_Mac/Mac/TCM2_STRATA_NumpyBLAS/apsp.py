"""TCM2: STRATA-Dense — NumPy BLAS matmul + Heaviside pruning (Mac).

Dense frontier propagation using Accelerate BLAS on macOS.
Same STRATA algebra as TCM1 but operating on dense numpy arrays.
"""
...
class DenseStrataIterator:
    """Dense BLAS-based reachability iterator (STRATA-Dense).
...
def run_apsp(A_csr, k=-1, verbose=True):
    """APSP via STRATA dense (Accelerate BLAS matmul on Mac)."""
