...
\begin{axis}[
  xbar, width=0.88\textwidth, height=7cm, bar width=9pt,
  xlabel={Facebook 실행 시간 (초, log scale)},
  symbolic y coords={TC1,TC2,TC3,BC2,BC4,BC5,BC3,BC1},
  ytick=data,
  yticklabels={D-STORM-SpMM-Cython,D-STORM-NumpyBLAS,D-STORM-GraphBLAS,SciPy,M-AORM,GB-bfs,I-AORM,NetworkX},
...
\end{axis}
\end{tikzpicture}
\caption{위상별 CPU 기법 --- Facebook 대표 비교 (log scale). TC1이 모든 위상에서 CPU 최고.}
\label{fig:topo-cpu}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{위상별 GPU 기법 --- Facebook 대표 비교 (log scale). BG1/BG2/TG2가 Tier~1.}
\label{fig:topo-gpu}
\end{figure}
