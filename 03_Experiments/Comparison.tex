...
간선 삽입은 거리가 감소만 하므로 $D'_{ij} = \min(D_{ij}, D_{ia}+1+D_{bj})$로 $O(n^2)$에 해결된다. 반면 간선 삭제는 거리가 증가할 수 있어, 삭제된 간선이 최단 경로에 사용되었는지 판정하고 대체 경로를 탐색해야 한다. 다음은 D-STORM의 거리 행렬 $D$를 활용한 효율적 삭제 알고리즘이다.
