"""BC3: I-AORM — Incremental AORM (edge-wise, dense, Python loop).
...
class IAORMIterator:
    ...
    def __next__(self):
        ...
        # Edge-wise multiplication: for each node, sum neighbor rows
        temp = np.zeros((self.n, self.n))
        for node in range(self.n):
            temp[node, :] = self.Rk[self.neigh[node], :].sum(axis=0)

        # Shortest-path pruning: keep only new discoveries
